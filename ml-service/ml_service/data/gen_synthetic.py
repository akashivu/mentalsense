# ml-service/data/gen_synthetic.py
import random
import numpy as np
import pandas as pd
...
def make_sample(label):
    ...
    """
    n = random.randint(20, 120)   # length of sequence
    t = 0.0
    samples = []
    raw_chars = []

    for _ in range(n):
        # 2% chance of backspace
        ...
        # stressed users → faster & more variable typing
        if label == 1:
...
def generate_dataset(n_low=800, n_high=400):
    ...
    # Generate low-stress samples
    for _ in range(n_low):
        rows.append(make_sample(0))

    # Generate high-stress samples
